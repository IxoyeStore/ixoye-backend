...
export default {
  ...
  "0 * * * *": async ({ strapi }: { strapi: any }) => {
    // Sin try/catch aqui, un rechazo (p.ej. conexion a la DB caida tras un
    // rato de inactividad) queda como unhandled rejection y Node mata todo
    // el proceso: node-schedule invoca esta funcion sin encadenarle catch.
    try {
      ...
      strapi.log.error("[orders-cleanup] error al cancelar ordenes pendientes:", err);
    }
  },
};